...
class ExcelSpreadsheet {
    ...
    function _parse() {
        ...
        $code = ExcelUltility::v($data, $pos);
        $length = ExcelUltility::v($data, $pos + 2);
        $version = ExcelUltility::v($data, $pos + 4);
        $substreamType = ExcelUltility::v($data, $pos + 6);
            ...
            switch ($code) {
                case SPREADSHEET_EXCEL_READER_TYPE_SST:
                    ...
                    for ($i = 0; $i < $uniqueStrings; $i++) {
                        ...
                        if ($spos == $limitpos) {
                            $opcode = ExcelUltility::v($data, $spos);
                            $conlength = ExcelUltility::v($data, $spos + 2);
                            if ($opcode != 0x3c) {
                                return -1;
                            }
                            $spos += 4;
                        ...
                        if ($richString) {
                            // Read in the crun
                            $formattingRuns = ExcelUltility::v($data, $spos);
                            $spos += 2;
                        }
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_FORMAT:
                    $indexCode = ExcelUltility::v($data, $pos + 4);
                    if ($version == SPREADSHEET_EXCEL_READER_BIFF8) {
                        $numchars = ExcelUltility::v($data, $pos + 6);
                        if (ord($data[$pos + 8]) == 0) {
                ...
                case SPREADSHEET_EXCEL_READER_TYPE_FONT:
                    $height = ExcelUltility::v($data, $pos + 4);
                    $option = ExcelUltility::v($data, $pos + 6);
                    $color = ExcelUltility::v($data, $pos + 8);
                    $weight = ExcelUltility::v($data, $pos + 10);
...
//						$bgcolor = ($bgi & 0x3f80) >> 7;
    ...
    function _getCellDetails($spos, $numValue, $column) {
        ...
        if ($type == 'date') {
            ...
            $dateinfo = ExcelUltility::gmgetdate($utcValue);
