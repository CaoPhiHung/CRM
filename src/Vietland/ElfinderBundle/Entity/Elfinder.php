...
namespace Vietland\ElfinderBundle\Entity;
...
/**
 * Vietland\ElfinderBundle\Entity\Elfinder
 */
class Elfinder {
    ...
    protected $_options = array(
        ...
        'disabled' => array('mkdir', 'mkfile', 'duplicate', 'archives', 'extract'), // list of not allowed commands
        ...
        'uploadDeny' => array('text/x-php', 'text/x-python', 'text/x-java-source', 'text/x-ruby', 'text/x-shellscript', 'text/x-perl', 'text/x-sql'), // mimetypes which not allowed to upload
    ...
    /**
     *
     * @param type $path to read
     * @return int number of files
     */
    private function _countFiles($FolderPath) {
        $filescount = 0;
        // Open the directory
        ...
        for ($i = 0, $s = count($_FILES['upload']['name']); $i < $s; $i++) {
            if (!empty($_FILES['upload']['name'][$i])) {
                ...
                if ($_FILES['upload']['error'][$i] > 0) {
                    ...
                    $name = str_replace('_', '', $this->_checkName($_FILES['upload']['name'][$i]));
    ...
    private function cropImage($image, $thumb_size = 64, $jpg_quality = 50) {
